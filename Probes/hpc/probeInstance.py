...
CONF_FILE = "/etc/serrano/hpc_probe.yaml"
...
class ProbeInstance:
    ...
    def __init__(self, conf):
        ...
        self.probeInterface = hpcProbe.HPCProbe(self.__probe_uuid, self.__config["hpc"])
    ...
    def __probe_registration(self):
        ...
        agent_url = "http://%s:%s" % (self.__config["probe_interface"]["exposed_address"],
                                      self.__config["probe_interface"]["exposed_port"])

        request_url = "http://%s:%s" % (self.__config["telemetry_handler"]["address"],
        ...
        request_data = {"cluster_uuid": self.__cluster_uuid, "probe_uuid": self.__probe_uuid, "streaming_telemetry": 0,
                        "url": agent_url, "type": "Probe.HPC", "inventory": self.probeInterface.get_inventory_data()}
